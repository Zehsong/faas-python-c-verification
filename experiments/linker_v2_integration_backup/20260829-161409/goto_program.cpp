...
bool esbmc_parseoptionst::read_goto_binary(goto_functionst &goto_functions)
{
  ...
  const irep_idt py_init_entry = "c:@F@__ESBMC_PY_INIT_ENTRY";
  ...
  for (const auto &arg : cmdline.args)
  {
    ...
    if (first_binary && cmdline.isset("equiv-py-target"))
    {
      ...
      goto_functions.function_map[py_init_entry] = py_main->second;
      ...
      goto_functions.function_map[py_init_entry]
        .update_instructions_function(py_init_entry, true);

      log_status(
        "Relational bridge: preserved Python initialization entry as `{}`",
        id2string(py_init_entry));
    }
