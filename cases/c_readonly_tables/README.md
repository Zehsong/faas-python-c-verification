...
此 schema 1 阶段接纳的是**函数内、固定长度、完整字面量初始化的只读数组**。
局部可变数组、全局表、任意指针和缓存状态尚未接入；这不是整个 M3 已完成。
数组参数及其输出已另行加入 [schema 2 接口](../c_bounded_arrays/README.md)，有独立待运行验收。
...
有界数组输入/输出的独立内存副本和观察约定见 schema 2 指南；可变缓存状态仍未由通用接口覆盖。
